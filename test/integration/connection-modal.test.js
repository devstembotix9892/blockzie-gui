...
// The tests below require blockzie Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying blockzie Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('blockzie Link'); // blockzie Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying blockzie Link is unavailable (BT)', async () => {
        await loadUri(uri);
